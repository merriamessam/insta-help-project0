...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (type === 'doc') {
      this.setState({ isDoc: true, isAuth: true, token, userId });
    } else if (type === 'patient') {
      this.setState({ isPatient: true, isAuth: true, token, userId });
    }
  ...
  mobileNavHandler = (isOpen) => this.setState({ showMobileNav: isOpen, showBackdrop: isOpen });
  backdropClickHandler = () => this.setState({ showBackdrop: false, showMobileNav: false, error: null });
  ...
  patientLoginHandler = (event, authData) => {
    ...
    const options = {
      ...
      data: { email: authData.email, password: authData.password },
    };
    Axios(options)
      .then((resData) => {
        let data = { ...resData.data.loadedUser };
        let patient = { name: data.name, email: data.email };
        ...
        this.setState({ isAuth: true, isPatient: true, token: resData.data.token, userId: resData.data.userId });
        this.setAutoLogout(remainingMilliseconds);
      })
      .catch((err) => this.setState({ isAuth: false, isPatient: false, error: err }));
  };
  ...
  docLoginHandler = (event, authData) => {
    ...
    Axios(options)
      .then((resData) => {
        ...
        const expiryDate = new Date(new Date().getTime() + remainingMilliseconds);
        localStorage.setItem('expiryDate', expiryDate.toISOString());
        this.setState({ isAuth: true, isDoc: true, token: resData.data.token, userId: resData.data.userId });
        this.setAutoLogout(remainingMilliseconds);
      })
      .catch((err) => this.setState({ isAuth: false, isDoc: false, error: err }));
  };
  ...
  patientSignupHandler = (event, authData) => {
    ...
    Axios(options)
      ...
      .catch((err) => this.setState({ isAuth: false, error: err }));
  };
  ...
  render() {
    ...
    let routes = (
      <Switch>
        ...
        <Route path="/docSignup" exact render={(props) => <DocSignup {...props} onSignup={this.docSignupHandler} />} />
        <Route path="/patientSignup" exact render={(props) => <PatientSignup {...props} onSignup={this.patientSignupHandler} />} />
        <Route path="/login" exact><Login /></Route>
        <Route path="/docLogin" exact render={(props) => <DocLogin {...props} onLogin={this.docLoginHandler} />} />
        <Route path="/patientLogin" exact render={(props) => <PatientLogin {...props} onLogin={this.patientLoginHandler} />} />
    ...
    if (this.state.isAuth && this.state.isDoc) {
      ...
      routes = (
        <Switch>
          <Route exact path="/"><DocDashboard /></Route>
          <Route exact path="/doctorProfile"><DocProfile /></Route>
          <Route exact path="/prescription"><PrescriptionPanel /></Route>
          <Route exact path="/invitation"><InvitationPanel /></Route>
          <Route exact path="/prescribePage"><PrescribePage /></Route>
          <Route exact path="/videodoctorPanel"><VideoRequest /></Route>
          <Route path="/patientProfile/:id" render={(props) => <InvProfile {...props} />} />
          <Route path="/details/:id" render={(props) => <PatientMonitored {...props} />} />
        </Switch>
      );
    }
    ...
    if (this.state.isAuth && this.state.isPatient) {
      ...
      routes = (
        <Switch>
          <Route exact path="/"><PatDashboard /></Route>
          <Route exact path="/patientProfile"><PatProfile /></Route>
          <Route exact path="/doctorPanel"><DoctorPanel /></Route>
          <Route exact path="/selfMonitor"><MonitorPat /></Route>
          <Route exact path="/prescription"><PatientPrescriptionPanel /></Route>
          <Route exact path="/prescribePage"><PatientPrescribePage /></Route>
          <Route exact path="/diet"><Diet /></Route>
        </Switch>
      );
    }

    const myStyle = { marginTop: "9vh", marginLeft: "7vh", marginRight: "5vh" };

    return (
      <Fragment>
        <Location stateHandler={(name) => this.setState({ stateName: name })} />
        {this.state.error && (
        ...
        >
          ...
          <ChatButton
            ...
            clickHandler={() => this.setState({ chat: true })}
          />
          <ChatBot
            show={this.state.chat}
...
const mapDispatchToProps = (dispatch) => ({ setPatient: (patient) => dispatch(setPatient(patient)) });
